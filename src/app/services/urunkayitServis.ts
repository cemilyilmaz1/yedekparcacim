...
export class urunkayitServis {
    ...
    KayitListeleFiltreli(marka: string) {
        return this.db.list("urunkayit", q => q.orderByChild("aracmarka").equalTo(marka));
    }
    KayitListelebyUid2(id: string) {
        return this.db.list("urunkayit", q => q.orderByChild("isletmeid").equalTo(id));
    }
    /* kayitlar bitiş */
}
